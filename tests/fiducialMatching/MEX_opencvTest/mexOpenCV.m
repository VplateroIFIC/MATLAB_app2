...
if ispc
    ...
    % Get the path for libmwocvmex.lib from Computer Vision Toolbox
    libDir = 'microsoft';
    linkLibPathMS = fullfile(matlabroot,'extern','lib',computer('arch'),libDir);
    cvstocvutil.sharedLibraries = linkLibPathMS;
else
    % Get the path for opencv .so/.dylib files from Computer Vision Toolbox
    ocvconfig.sharedLibraries = fullfile(matlabroot, 'bin', arch);
    % Get the path for libmwocvmex{.so, .dylib} from Computer Vision Toolbox
