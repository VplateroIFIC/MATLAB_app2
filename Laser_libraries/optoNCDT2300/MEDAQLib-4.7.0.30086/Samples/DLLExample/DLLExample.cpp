...
/* The quintessence of this program is:
	...
	CREATESENSORINSTANCE pCreateSensorInstance=   reinterpret_cast<CREATESENSORINSTANCE>  (GetProcAddress (h, "CreateSensorInstance"));
	RELEASESENSORINSTANCE pReleaseSensorInstance= reinterpret_cast<RELEASESENSORINSTANCE> (GetProcAddress (h, "ReleaseSensorInstance"));
	// ... all other functions ...

	DWORD sensor= pCreateSensorInstance (SENSOR_ILD2300);

	pOpenSensorIF2008_ETH (sensor, "169.254.168.150", 0);
	
	pExecSCmdGetDouble (sensor, "Get_Info" "SA_Range", &range);

	for (int i=0 ; i<10 ; i++)
		{
		double data;

		pPoll (sensor, NULL, &data, 1);
		Sleep (100);
		}
...
int Open (DWORD sensor)
	...
	CHECK_ERROR (pOpenSensorIF2008_ETH (sensor, _T("169.254.168.150"), 0));
	return ERR_NOERROR;
	}
...
int GetInfo (DWORD sensor)
	{
	double range;
	CHECK_ERROR (pExecSCmdGetDouble (sensor, _T("Get_Info"), _T("SA_Range"), &range));
