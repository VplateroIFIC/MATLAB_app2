function [X,Y,Z] = Laser_measure2(laser,gantry,Corner1,Corner2,nLines,velocity,name)
...
%    Start(gantry.vectorZ1,gantry.vectorU) = NaN;
    ...
    for i = 1:nLines                                    %For each X position
        ...
        gantry.MoveTo(gantry.Y,Y(i),velocity,1);
            if mod(i,2)                             %Odd line
                line=measureLineOK_Sento(laser,gantry,x(1),Y(i),x(1),Y(i),velocity,velocity+10);
%                 Start = x(1);
%                 Stop = x(2);

            else                                    %Even line
                line2=measureLineOK_Sento(laser,gantry,x(2),Y(i),x(1),Y(i),velocity,velocity+10);
                line = fliplr(line2);
            end
                %try
%                     line=measureLineOK_Sento(laser,gantry,x(1),Y(i),x(2),Y(i),velocity,velocity+10);
            ...
            measuresFileName = sprintf("%s_%dlines_%dmmseg.mat",name, pointsPerLine, velocity);
